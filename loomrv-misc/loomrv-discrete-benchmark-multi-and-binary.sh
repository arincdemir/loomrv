...
# Multi-property AND-merged binary benchmark for loomrv (discrete).
# Mirrors loomrv-discrete-benchmark-multi-and.sh but uses a .row.bin trace.
#
# Usage: ./loomrv-discrete-benchmark-multi-and-binary.sh [TRACE_SIZE]
set -xeuo pipefail
mkdir -p traces tmp
...
echo "Running loomrv multi-property AND discrete binary benchmark for commit ${commit_hash}"
...
hyperfine \
    ...
    --command-name "Combined_Discrete_MultiProperty_AND_30_binary" \
