#!/usr/bin/env bash
# rybinx-benchmark-multi.sh
# Sequential multi-property binary benchmark for rybinx.
# Mirrors ryjson-benchmark-multi.sh but uses a .row.bin trace.
#
# Usage: ./rybinx-benchmark-multi.sh [TRACE_SIZE [MAX_STEP]]
set -xeuo pipefail
mkdir -p traces tmp
...
echo "Running rybinx sequential multi-property binary benchmark for commit ${commit_hash}"
...
hyperfine \
    ...
    --command-name "rybinx_Sequential_30" \
