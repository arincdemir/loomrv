...
echo "Running loomrv sequential multi-property discrete binary benchmark for commit ${commit_hash}"

TRACE_SIZE="${1:-1000000}"
TEST_TRACE_JSONL="traces/generated_discrete_trace_${TRACE_SIZE}_s.jsonl"
TEST_TRACE_BIN="traces/generated_discrete_trace_${TRACE_SIZE}_s.row.bin"

if [ ! -f "${TEST_TRACE_JSONL}" ]; then
    echo "Generating discrete trace with size ${TRACE_SIZE}..."
    python3 tools/generate_traces.py -n "${TRACE_SIZE}" --step-mode constant --with-s -o "${TEST_TRACE_JSONL}"
fi

if [ ! -f "${TEST_TRACE_BIN}" ]; then
    echo "Converting ${TEST_TRACE_JSONL} to binary..."
    python3 tools/to_binary_row.py --file "${TEST_TRACE_JSONL}"
fi
...
WRAPPER_SCRIPT="tmp/run_doverify_discrete_seq_binary.sh"
...
while IFS= read -r formula; do
    ...
    $BIN_EXEC --binary --discrete "$TRACE_FILE" "${TEMP_PROP}" > /dev/null
done < "$PROPS_FILE"
OEF
...
hyperfine \
    ...
    --command-name "loomrv_Discrete_Sequential_Binary_30" \
        "./${WRAPPER_SCRIPT} \"${TEST_TRACE_BIN}\" \"${PROPS_FILE}\" \"${BIN_PATH}\""
