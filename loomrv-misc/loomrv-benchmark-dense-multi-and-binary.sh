#!/usr/bin/env bash
# loomrv-benchmark-multi-and-binary.sh
# Multi-property AND-merged binary benchmark for loomrv.
# Mirrors loomrv-benchmark-multi-and.sh but uses a .row.bin trace.
#
# Usage: ./loomrv-benchmark-multi-and-binary.sh [TRACE_SIZE [MAX_STEP]]
set -xeuo pipefail
mkdir -p traces tmp
...
echo "Running loomrv multi-property AND binary benchmark for commit ${commit_hash}"
...
hyperfine \
    ...
    --command-name "Combined_MultiProperty_AND_30_binary" \
