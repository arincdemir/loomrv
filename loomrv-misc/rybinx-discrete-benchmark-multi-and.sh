...
# AND-merged multi-property binary benchmark for rybinx (discrete).
# Mirrors ryjson-discrete-benchmark-multi-and.sh but uses a .row.bin trace.
#
# Usage: ./rybinx-discrete-benchmark-multi-and.sh [TRACE_SIZE]
set -xeuo pipefail
mkdir -p traces tmp
...
echo "Running rybinx single AND multi-property discrete binary benchmark for commit ${commit_hash}"
