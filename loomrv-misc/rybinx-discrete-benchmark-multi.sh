...
# Sequential multi-property binary benchmark for rybinx (discrete).
# Mirrors ryjson-discrete-benchmark-multi.sh but uses a .row.bin trace.
#
# Usage: ./rybinx-discrete-benchmark-multi.sh [TRACE_SIZE]
set -xeuo pipefail
mkdir -p traces tmp
...
echo "Running rybinx sequential multi-property discrete binary benchmark for commit ${commit_hash}"
...
hyperfine \
    ...
    --command-name "rybinx_Discrete_Sequential_30" \
