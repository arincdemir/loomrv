...
echo "====================================================="
echo "   Synthetic Multi-Property Deduplication Benchmark  "
echo "====================================================="

FORMULA_COUNT=${1:-10}
TRACE_LENGTH=${2:-1000000}
MAX_STEP=${3:-8}
TRACE_FILE="traces/synthetic_${TRACE_LENGTH}_${MAX_STEP}.jsonl"
...
if [ ! -f "${TRACE_FILE}" ]; then
    ...
    python3 tools/generate_traces.py --length "${TRACE_LENGTH}" --max-step "${MAX_STEP}" --out "${TRACE_FILE}" --step-mode random
else
    echo "Using existing trace: ${TRACE_FILE}"
fi
...
RYJSON_WRAPPER="tmp/run_ryjson_seq_synthetic.sh"
...
while IFS= read -r formula; do
    [ -z "$formula" ] && continue
    ryjson -v "$formula" "$TRACE" > /dev/null
done < "$PROPS"
EOF
chmod +x "${RYJSON_WRAPPER}"

# Wrapper for loomrv seq
DOVERIFY_WRAPPER="tmp/run_doverify_seq_synthetic.sh"
...
while IFS= read -r formula; do
    ...
    $BIN --dense "$TRACE" "$TMP_DIR/prop_$i.txt" > /dev/null
    i=$((i+1))
done < "$PROPS"
EOF
...
for SCENARIO in "${SCENARIOS[@]}"; do
    ...
    RES_FILE="${RESULTS_DIR}/bench_${SCENARIO_NAME}_${FORMULA_COUNT}props_${TRACE_LENGTH}ev_${commit_hash}.json"
    ...
    hyperfine \
        ...
        --command-name "loomrv (Multi-Property)" \
            "${BIN_PATH} --dense ${TRACE_FILE} ${PROPS_FILE}"
done
