...
echo "Running loomrv sequential multi-property binary benchmark for commit ${commit_hash}"

TRACE_SIZE="${1:-1000000}"
MAX_STEP="${2:-8}"
TEST_TRACE_JSONL="traces/generated_trace_${TRACE_SIZE}_${MAX_STEP}_s.jsonl"
TEST_TRACE_BIN="traces/generated_trace_${TRACE_SIZE}_${MAX_STEP}_s.row.bin"

if [ ! -f "${TEST_TRACE_JSONL}" ]; then
    echo "Generating trace with size ${TRACE_SIZE} and max_step ${MAX_STEP}..."
    python3 tools/generate_traces.py -n "${TRACE_SIZE}" --max-step "${MAX_STEP}" --with-s -o "${TEST_TRACE_JSONL}"
fi
...
WRAPPER_SCRIPT="tmp/run_doverify_seq_binary.sh"
...
while IFS= read -r formula; do
    ...
    $BIN_EXEC --binary --dense "$TRACE_FILE" "${TEMP_PROP}" > /dev/null
done < "$PROPS_FILE"
OEF
...
hyperfine \
    ...
    --command-name "loomrv_Sequential_Binary_30" \
