#!/usr/bin/env bash
# loomrv-benchmark-multi-binary.sh
# Multi-property binary benchmark for loomrv.
# Mirrors loomrv-benchmark-multi.sh but uses a .row.bin trace.
#
# Usage: ./loomrv-benchmark-multi-binary.sh [TRACE_SIZE [MAX_STEP]]
set -xeuo pipefail
mkdir -p traces tmp
...
echo "Running loomrv multi-property binary benchmark for commit ${commit_hash}"
...
hyperfine \
    ...
    --command-name "Combined_MultiProperty_30_binary" \
