...
echo "Running loomrv sequential multi-property benchmark for commit ${commit_hash}"
...
WRAPPER_SCRIPT="tmp/run_doverify_seq.sh"
...
while IFS= read -r formula; do
    ...
    $BIN_EXEC --dense "$TRACE_FILE" "${TEMP_PROP}" > /dev/null
done < "$PROPS_FILE"
OEF
